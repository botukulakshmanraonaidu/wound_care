...
import { LayoutDashboard, Users, FileText, Settings, Bell, LogOut, Activity, Database, Shield, X } from 'lucide-react';
...
function Sidebar({ onLogout, userRole: rawRole, accessLevel, isOpen, onClose }) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="md:hidden hidden fixed inset-0 z-40 bg-black/40 backdrop-blur-sm transition-opacity"
          onClick={onClose}
          aria-hidden="true"
        />
      )}

      <aside className={`sidebar fixed sm:static inset-y-0 left-0 z-50 w-56 bg-white shadow-xl sm:shadow-none transform transition-transform duration-300 ease-in-out sm:translate-x-0 flex flex-col h-full border-r border-gray-100 ${isOpen ? 'translate-x-0' : '-translate-x-full sm:translate-x-0'
        }`}>
                  ...
                  <div className="alerts-list">
                    {notifications.map(notif => (
                      <div
                        ...
                        onClick={() => {
                          onClose(); // Auto-dismiss sidebar on alert click
